...
export function WebGLKnowledgeExplorer({ onUseSvg }: { onUseSvg: () => void }) {
  ...
  return (
    <div className="relative h-full overflow-hidden bg-bg-deep">
      <KnowledgeScene className="absolute inset-0" onNodeSelect={setSelectedId} />
      ...
      <div className="absolute right-6 top-6 z-10 flex gap-2">
        <button type="button" onClick={() => window.location.reload()} className="inline-flex items-center gap-1.5 rounded-md border border-border-subtle/60 bg-surface-elevated/80 px-2.5 py-1.5 text-xs text-text-secondary hover:text-text-primary"><RotateCcw size={13} /> Ajustar visão</button>
